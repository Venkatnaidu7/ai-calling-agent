...
from app.models import HumanAgent, RoutingGroup, TransferDestination, BusinessHours, CallHandoff
...
async def select_destination(db: AsyncSession, tenant_id, routing_group_id=None):
    if routing_group_id:
        ...
        if group.strategy == 'ROUND_ROBIN':
            return await db.scalar(select(HumanAgent).where(HumanAgent.tenant_id == tenant_id, HumanAgent.status.in_(ACTIVE_STATES)).order_by(HumanAgent.updated_at.asc(), HumanAgent.priority.desc()).limit(1))
        return await db.scalar(select(HumanAgent).where(HumanAgent.tenant_id == tenant_id, HumanAgent.status.in_(ACTIVE_STATES)).order_by(HumanAgent.priority.desc(), HumanAgent.updated_at.asc()).limit(1))
...
async def is_business_hours(db: AsyncSession, tenant_id, now: datetime | None = None) -> bool:
    ...
    for window in rule:
        if isinstance(window, (list, tuple)) and len(window) == 2 and str(window[0]) <= current <= str(window[1]):
            return True
    return False
...
async def create_or_get_handoff(db: AsyncSession, tenant_id, call, destination, reason=None):
    ...
    if handoff:
        return handoff
    provider = (await db.scalar(select(__import__('app.models', fromlist=['PhoneNumber']).PhoneNumber.provider).where(__import__('app.models', fromlist=['PhoneNumber']).PhoneNumber.id == call.phone_number_id))) or 'twilio'
