...
from app.models import CallIntelligenceJob
...
def process_call_summary(self, call_id: str):
    return process_call_intelligence.delay(call_id).get(disable_sync_subtasks=False)

@celery.task(bind=True, max_retries=4, acks_late=True)
def process_call_intelligence(self, call_id: str):
    async def run():
        async with SessionLocal() as db:
            job = await db.scalar(select(CallIntelligenceJob).where(CallIntelligenceJob.call_id == call_id))
            if job:
                job.status='PROCESSING'; job.attempts += 1; await db.commit()
            try:
                result = await analyze_call(db, job.tenant_id if job else None, call_id) if job else None
                return {'status':'completed' if result else 'skipped','call_id':call_id}
            except IntelligenceError as exc:
                if job:
                    job.status='RETRY' if self.request.retries < self.max_retries else 'FAILED'; job.last_error=str(exc); await db.commit()
                raise
    import asyncio
    try:
