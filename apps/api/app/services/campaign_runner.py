...
async def run_campaign(campaign_id: str, tenant_id: str):
    ...
    async with SessionLocal() as db:
        ...
        phone_value = (campaign.schedule or {}).get('phone_number_id')
        if not phone_value:
            return {'status': 'blocked', 'reason': 'PHONE_NUMBER_REQUIRED'}
        try:
            phone_uuid = UUID(str(phone_value))
        except (ValueError, TypeError):
            return {'status': 'blocked', 'reason': 'PHONE_NUMBER_INVALID'}
        ...
        rows = (await db.scalars(
            ...
            .where(CampaignContact.campaign_id == cid, CampaignContact.tenant_id == tid, CampaignContact.state.in_(['QUEUED', 'RETRY']))
            .order_by(CampaignContact.created_at.asc())
            .limit(campaign.concurrency)
        ...
        for item in rows:
            ...
            try:
                ...
                item.state = 'RETRY' if item.attempts < int((campaign.retry_policy or {}).get('max_attempts', 3)) else 'FAILED'
        await db.commit()
