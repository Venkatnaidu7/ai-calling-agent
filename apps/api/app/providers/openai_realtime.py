...
class RealtimeBridge:
    ...
    def __init__(self, instructions, voice="marin", language="en", tools=None):
        self.instructions = instructions or "You are a helpful phone assistant."
        self.voice = voice or "marin"
    ...
    async def send_audio(self, payload):
        if self.ws:
            await self.ws.send(json.dumps({"type": "input_audio_buffer.append", "audio": payload}))
    ...
    async def create_response(self):
        if self.ws:
            await self.ws.send(json.dumps({"type": "response.create", "response": {"output_modalities": ["audio"]}}))
