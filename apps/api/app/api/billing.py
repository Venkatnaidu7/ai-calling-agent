...
async def stripe_webhook(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    # The unique Stripe event id is the concurrency guard. A savepoint lets a
    # concurrent duplicate roll back only the insert, without poisoning the
    # request transaction.
    try:
    ...
    await db.commit()
    return {'received': True, 'event_id': event_id, 'duplicate': False}
