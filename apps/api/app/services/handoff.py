...
def _window_contains(rule, current: str) -> bool:
    if isinstance(rule, dict): rule = [rule]
    if not isinstance(rule, list): return False
    for window in rule:
        if isinstance(window, dict): start, end = window.get('start'), window.get('end')
        elif isinstance(window, (list, tuple)) and len(window) == 2: start, end = window
        else: continue
        if not isinstance(start, str) or not isinstance(end, str): continue
        if start <= end and start <= current <= end: return True
        if start > end and (current >= start or current <= end): return True
    return False
...
async def is_business_hours(db: AsyncSession, tenant_id, now: datetime | None = None) -> bool:
    ...
    if not bh: return True
    try: zone = ZoneInfo(bh.timezone or 'UTC')
    ...
    if _window_contains(hours.get(local.strftime('%a').lower()), current): return True
    previous = local - timedelta(days=1)
    previous_rule = hours.get(previous.strftime('%a').lower())
    if isinstance(previous_rule, dict): previous_rule = [previous_rule]
    if isinstance(previous_rule, list):
        for window in previous_rule:
            if isinstance(window, dict): start, end = window.get('start'), window.get('end')
            elif isinstance(window, (list, tuple)) and len(window) == 2: start, end = window
            else: continue
            if isinstance(start, str) and isinstance(end, str) and start > end and current <= end: return True
    return False
...
async def create_or_get_handoff(db: AsyncSession, tenant_id, call, destination, reason=None):
    ...
    provider = (await db.scalar(select(PhoneNumber.provider).where(PhoneNumber.id == call.phone_number_id, PhoneNumber.tenant_id == tenant_id)) or '').lower()
    if provider not in VOICE_PROVIDERS: raise HTTPException(400, 'Unsupported voice provider')
    ...
    try:
        ...
        existing = await db.scalar(select(CallHandoff).where(CallHandoff.call_id == call.id, CallHandoff.tenant_id == tenant_id))
...
async def start_transfer(db: AsyncSession, tenant_id, call, destination, reason=None):
    ...
    if handoff.status in {'TRANSFERRING', 'CONNECTED'}: return handoff
    if handoff.status == 'FAILED': raise HTTPException(409, 'Human handoff has already failed for this call')
    ...
    try:
        if provider == 'twilio':
            ...
            TwilioClient(s.twilio_account_sid, s.twilio_auth_token).calls(call.provider_call_id).update(url=f'{base}/api/v1/handoff/twilio/execute/{call.id}', method='POST')
        elif provider == 'plivo':
            s = get_settings()
            if not s.plivo_auth_id or not s.plivo_auth_token: raise HTTPException(503, 'Plivo is not configured')
            PlivoClient(s.plivo_auth_id, s.plivo_auth_token).calls.update(call.provider_call_id, legs='aleg', aleg_url=f'{base}/api/v1/handoff/plivo/execute/{call.id}', aleg_method='POST')
        else: raise HTTPException(400, 'Unsupported voice provider')
    except HTTPException: raise
    except Exception as exc:
        handoff.status = 'FAILED'; handoff.failure_reason = str(exc)[:1000]; await db.flush()
        raise HTTPException(502, f'Unable to start {provider} human transfer')
    handoff.status = 'TRANSFERRING'; call.outcome = 'HUMAN_HANDOFF'; await db.flush()
    return handoff
...
async def sync_handoff_state(db: AsyncSession, call, provider_status: str):
    ...
    if not handoff: return None
    status = (provider_status or '').upper().replace(' ', '-')
    if handoff.status in {'FAILED', 'CONNECTED'}: return handoff
    if status in {'ANSWERED', 'IN-PROGRESS', 'IN_PROGRESS'}:
        handoff.status = 'CONNECTED'; handoff.failure_reason = None
        ...
        handoff.status = 'FAILED'; handoff.failure_reason = status
    return handoff
