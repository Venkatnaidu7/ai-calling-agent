...
async def status(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    continue_campaign_id = None; retry_delay = 0
        ...
        if attempt:
            ...
            if campaign and item:
                ...
                if new_status == 'COMPLETED': item.state, attempt.state = 'COMPLETED', 'COMPLETED'
                    ...
                    item.state, attempt.state, attempt.error_code = 'RETRY', 'RETRY', new_status
                    retry_delay = max(0, int((campaign.retry_policy or {}).get('backoff_minutes', 0))) * 60
    ...
    try:
        ...
        async def ai_loop():
            async for event in bridge.events():
                ...
                elif kind == 'error': raise RuntimeError(event.get('error',{}).get('message','OpenAI Realtime error'))
        while True:
        ...
        try:
            async with SessionLocal() as db:
                ...
                if call and call.status in {'QUEUED','RINGING','IN_PROGRESS'}:
                    call.status = 'COMPLETED'; call.ended_at = datetime.now(timezone.utc); await db.commit()
        except Exception: pass
